...
def build_system_prompt(
    *,
    ...
    selected_domain = str(domain or "software").strip().lower()
    personas = (
        persona_config.RIEMANN_PERSONAS
        if selected_domain == "riemann"
        else persona_config.LENS_PERSONAS
    )
    ...
    if selected_domain == "riemann":
        heading = "數學研究鏡頭"
        first_paragraph = (
            f"這一輪借用 {personas[persona]} 的核心概念、研究操作與關注面向，"
            "決定如何整理證據、從哪裡看這段研究。\n"
        )
        final_line = "不是扮演或模仿人物，也不是增加一份完整 proof review。\n"
    else:
        heading = "工作流觀察鏡頭"
        first_paragraph = (
...
def _close_reaction(text: str, max_chars: int, *, domain: str = "software") -> str:
    ...
    math_symbols = re.compile(r"[=<>≤≥∑∫∏∀∃∞]|\\(?:sum|int|prod|forall|exists)|\$[^$]+\$")
    if str(domain).strip().lower() == "riemann" and math_symbols.search(clipped):
        compacted = clipped
...
def sanitize_reaction(
    ...
    *,
    ...
    return _close_reaction(text, max_chars, domain=domain)
