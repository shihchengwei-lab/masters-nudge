...
LENS_BADGES = {
    ...
    "riemann": (persona_config.persona_label("riemann"), "#4FD1C5"),
    "ramanujan": (persona_config.persona_label("ramanujan"), "#F6C85F"),
    "erdos": (persona_config.persona_label("erdos"), "#FF7A90"),
    "tao": (persona_config.persona_label("tao"), "#63A4FF"),
    "selberg": (persona_config.persona_label("selberg"), "#B794F4"),
    "polya": (persona_config.persona_label("polya"), "#FF9F43"),
...
def riemann_stage_label(stage: str) -> str:
    persona = riemann_router.STAGE_LENSES[stage]
    return (
        f"{RIEMANN_STAGE_NAMES[stage]} · "
        f"{persona_config.PERSONA_NAMES[persona]}"
        f"（{persona_config.PERSONA_FOCUS[persona]}）"
    )


def selector_options(domain: str = "software") -> list[str]:
    if domain == "riemann":
        return [riemann_stage_label(stage) for stage in riemann_router.STAGE_LENSES]
...
RIEMANN_SELECTOR_STAGES = {
    riemann_stage_label(key): key for key in riemann_router.STAGE_LENSES
}


def stage_selection_label(
    selection: persona_config.StageSelection, domain: str = "software"
) -> str:
    """Describe lifecycle, forced specialist, and legacy selections accurately."""
    if domain == "riemann" and selection.stage in riemann_router.STAGE_LENSES:
        return riemann_stage_label(selection.stage)
...
class BuddyWindow:
    def __init__(self, root: tk.Tk):
        ...
        self.profile_session = SessionRef(
            "codex_cli", "window", cwd=cwd, repo_root=repo_root
        )
        ...
        if profile.source == "workspace_profile" and profile.domain == "riemann":
            self.stage_selection = persona_config.StageSelection(
                profile.stage,
                riemann_router.STAGE_LENSES[profile.stage],
                "workspace_profile",
                False,
            )
        else:
    ...
    def _build_ui(self):
        ...
        selected_label = stage_selection_label(self.stage_selection, self.domain)
        ...
        initial_text = "( . . . )"
        if self.domain == "riemann":
            initial_text = f"黎曼研究 profile：{selected_label}。"
    ...
    def _on_stage_selected(self, _event=None):
        ...
        stages = RIEMANN_SELECTOR_STAGES if self.domain == "riemann" else SELECTOR_STAGES
        ...
        if self.domain == "riemann":
            ...
            if not result["saved"]:
                self.bubble_label.config(
                    text="Workspace profile 無法儲存，仍使用原設定。"
                )
                return
            ...
            persona = riemann_router.STAGE_LENSES[stage]
            self.stage_selection = persona_config.StageSelection(
                stage, persona, "workspace_profile", False
            )
            self._set_lens_badge(persona)
            message = (
                f"下一次 review 起以 {label} 為 primary；自動鏡頭依本輪證據接手，"
                "連續五輪後會交還 primary 一輪。"
            )
