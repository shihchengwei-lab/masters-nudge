...
class SettingsPackageTests(unittest.TestCase):
    ...
    def test_provider_prompt_challenges_the_whole_change_before_inspecting_internals(self):
        ...
        required = normalized.index("required := min_relations")
        removal = normalized.index("alternatives(r) := { remove(r), replace(r), keep(r) }")
        whole_change = normalized.index("whole := best")
        internals = normalized.index("internal := best")
        checks = normalized.index("focus on these six checks")
        self.assertLess(goal, required)
        self.assertLess(required, removal)
        self.assertLess(removal, whole_change)
        self.assertLess(whole_change, internals)
        self.assertLess(internals, checks)
        self.assertIn("selected := argmax", normalized)
        self.assertIn("tie_break := earliest_required_choice", normalized)
    ...
    def test_provider_prompt_emits_the_selected_alternative_as_formal_relation(self):
        prompt = (ROOT / "buddy-prompt.txt").read_text(encoding="utf-8")
        normalized = " ".join(prompt.split())
        self.assertIn("alternatives(r) := { remove(r), replace(r), keep(r) }", normalized)
    ...
    def test_provider_prompt_speaks_when_two_concrete_relations_are_supported(self):
        prompt = (ROOT / "buddy-prompt.txt").read_text(encoding="utf-8")
        normalized = " ".join(prompt.split())
        self.assertIn("emit := feedback iff concrete(OBSERVED) && concrete(PREFER) && task_preserving(PREFER)", normalized)
    ...
    def test_provider_prompt_compares_candidates_before_selecting(self):
        prompt = (ROOT / "buddy-prompt.txt").read_text(encoding="utf-8")
        normalized = " ".join(prompt.split())
        whole = normalized.index("whole := best")
        internal = normalized.index("internal := best")
        supported = normalized.index("candidates := evidence_supported")
        choose = normalized.index("selected := argmax")
        self.assertLess(whole, internal)
        self.assertLess(internal, supported)
        self.assertLess(supported, choose)
        self.assertIn("evidence_supported({ whole, internal })", normalized)
