...
def tasks_from_records(contract: dict[str, Any], record_root: Path) -> list[dict[str, Any]]:
    rows: list[dict[str, Any]] = []
    for configured in contract["provisional_tasks"]:
        source = record_root / configured["task_key"]
        row = dict(load_json(source / "task.json"))
        row.update(configured)
        row["test_patch"] = (source / "test.patch").read_text(encoding="utf-8")
        rows.append(row)
    return rows
...
def reviewer_env(provider: str, model: str) -> dict[str, str]:
    provider = str(provider or "").strip().lower()
    model = str(model or "").strip()
    if not provider or not model:
        raise ValueError("reviewer provider and model must be explicit")
    return {
        "MASTERS_NUDGE_PROVIDER": provider,
        "MASTERS_NUDGE_MODEL": model,
...
def run_arm(
    ...
    env.update(reviewer_env(reviewer_provider, reviewer_model))
    ...
    result = {
        ...
        "reviewer_provider": reviewer_provider if arm == "B" else None,
        "reviewer_model": reviewer_model if arm == "B" else None,
...
def phase_run(
    ...
    for task in tasks:
        ...
        if not preflight_path.exists():
            ineligible.append(f"{task['instance_id']}: missing preflight")
            continue
        preflight = load_json(preflight_path)
        if preflight.get("status") != "eligible":
            ineligible.append(
                f"{task['instance_id']}: {preflight.get('status')} ({preflight.get('reason')})"
            )
    if ineligible:
        details = "\n".join(f"- {item}" for item in ineligible)
        raise RuntimeError(
            "Refusing to start any model arm until all preregistered tasks "
            f"are eligible or replaced by a recorded pre-arm amendment:\n{details}"
        )
    for task in tasks:
...
def phase_summarize(
    tasks: list[dict[str, Any]], work_root: Path, results_path: Path
) -> None:
    rows: list[dict[str, Any]] = []
    for task in tasks:
        root = task_dir(work_root, task)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--results-path", type=Path, default=HERE / "results.json")
    parser.add_argument("--reviewer-provider", default="openai")
    parser.add_argument("--reviewer-model", default="gpt-5.6-sol")
    parser.add_argument(
        "--task-record-root",
        type=Path,
        help="Reuse frozen task.json and test.patch records instead of loading Parquet",
    )
...
def main() -> int:
    ...
    if args.task_record_root:
        tasks = tasks_from_records(contract, args.task_record_root)
    else:
    ...
    for name, function in phases:
        ...
        if name == "run":
            ...
            function(tasks, args.work_root, args.results_path)
        else:
