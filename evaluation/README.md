...
Raw rows, screenshots, generated dashboards, zipped workspaces, stopped harnesses, and the full Riemann research/runtime snapshots are preserved in the verified [evidence archive release](https://github.com/shihchengwei-lab/masters-nudge/releases/tag/evidence-archive-2026-08-22).
...
The current executable Shader protocols and tools live under `shader_prompt_replay/`, `shader_candidate_search/`, `shader_long_tail/`, and `nudge_interaction/`. New runs must use a new explicit output directory and must not overwrite a retained result.

## Separate observational archive

The compact Riemann domain and observational benchmark remain under [experiment/riemann-domain](../experiment/riemann-domain/README.md). The full working research archive is available only from the evidence release above.
