...
- 讀取最近的 transcript 片段：從最新一則 user/assistant 開始往回填，
  總字數預算 6000 字（大致涵蓋 3 輪 round-trip，再多 Buddy 也無法
  對齊到自己過去的反應，反而會幻覺）；每則訊息整則保留，但單則上限
  5000 字（超過尾截 +「…」前綴）；最舊的一則若放不下整則，剩餘預算
  ≥ 400 字時會裁進來（尾截 +「…」），< 400 字就整則丟掉。`tool_result`
  內容從被選中的訊息範圍蒐集，串接後尾截到 2000 字、另外包成一個
  `[tool output]`
...
1. **最近的 user/assistant 訊息**，包在
   ...
   總字數預算 6000 字；每則整則保留，單則上限 5000 字（超過尾截
   +「…」前綴）；最舊的一則若放不下整則，剩餘預算 ≥ 400 字時會
   裁進來（尾截 +「…」），< 400 字就整則丟掉。單則超過上限時保留
   最新講的部分、前面的丟掉，開頭加「…」標記讓 reviewer 知道有東西
   被砍掉。
2. **一個尾段 `[tool output] … [end tool output]` 區塊**：上面被預算
   選中的訊息範圍內，所有 `tool_result` 串接後尾截到約 2000 字元。
