...
輸出。可見證據若支援契約缺口或一個尚未關閉的關鍵分岔，Provider 回傳
`contract_warning` 並停止本次品味判斷；沒有合格警告後，才可能回傳 `taste_nudge`。

三個原則各用一個詞表示：`validity` 是非法狀態、`causality` 是單向因果流、
`predictability` 是可預測性。Provider 分開回傳直接支撐判斷的可見 `evidence_seq`、
原則、精準定位程式概念的 `anchor`，
以及用一個短句描述或詢問單一工程關係的 `relationship`。證據足以建立工程關係時，
Provider 使用觀察句；證據只足以建立會改變實作的具體分岔時，Provider 詢問缺少的
一項事實；兩者都不成立時保持沉默。`anchor` 只保留一個最小定位。Host 把
...
每次輸出都依目前情況生成，不是隨機抽一句罐頭訊息。契約警告或品味提醒都是一則
觀察或一個能改變當前工程決策的精準問題，不是 review、評分、完整解法，也不是一律
要求多跑測試。Provider 沒有回傳契約警告，只表示目前封包沒有產生合格警告，不證明
整份任務契約已完成。
