...
> **測試通過只提供被測行為的證據；先檢查任務契約，再談設計品味。**

Masters’ Nudge 會在 Claude Code 或 Codex Agent 做下一個決定前，提供一則簡短、
有證據的契約警告或工程品味提醒。Masters’ Nudge 不會代替 Agent 解題或阻止 Agent
繼續；如何修正與驗證仍由主模型決定。
...
修改若明確指定工作區內的檔案，同一筆證據也會帶入修改行附近、受長度限制的目前
原始碼；不會尋找關聯檔案或保留原始碼歷史。Provider 先檢查可見的任務契約；沒有
合格的契約警告後，才檢查最可能改變下一個工程決策的結構取捨。
同一個工作階段中，最多三則已回傳的 Nudge 會另外作為去重排除資訊送入；這些 Nudge
不是證據，也不代表主 Agent 的決策軌跡。

Provider 在同一次判斷中先檢查任務契約，再用三個結構原則檢查工程品味：

- 讓非法狀態無法由資料結構建構；
- 讓事件與狀態沿單向因果流動；
- 讓副作用與依賴保持明確，使行為可以局部推理。

兩個階段與三個原則共同構成一次判斷，不是額外的 Provider 呼叫、三個 Lens 或三則
輸出。可見證據若能確立契約缺口，Provider 回傳 `contract_warning` 並停止本次品味
判斷；沒有合格警告後，才可能回傳 `taste_nudge`。品味提醒只陳述目前封包已直接
顯示的責任重疊：兩個具體程式元素共同承擔同一份狀態、行為、副作用或控制流程責任。
Actor 自行決定修法。

三個原則各用一個詞表示：`validity` 是非法狀態、`causality` 是單向因果流、
`predictability` 是可預測性。Provider 分開回傳直接支撐判斷的可見 `evidence_seq`、
原則、精準定位程式概念的 `anchor`，
以及用一個短句陳述契約破壞或可見責任重疊的 `relationship`。
缺少的上下文不會被當成契約警告或品味提醒。只有一個元素可見、需要追查遠處 caller、
跨檔案 owner 或生命週期才可能成立的關係會保持沉默。`anchor` 只保留一個最小定位。Host 把
`contract_warning` 顯示為 `causality warning:`，把 `taste_nudge` 顯示為
`causality nudge:`。`warning` 表示可見的契約仍有缺口或尚未關閉，不代表 Hook 會阻止
主模型繼續。

品味提醒不提問、不指示修改，也不把缺少的事實包裝成條件；修法仍由 Actor 決定。
例行驗證狀態與重述任務不屬於 Nudge 的角色。
...
每次輸出都依目前情況生成，不是隨機抽一句罐頭訊息。契約警告陳述可見的破壞；品味
提醒陳述封包內兩個具體元素的責任重疊，不是 review、評分、完整解法，也不是一律要求多跑測試。
Provider 沒有回傳契約警告，只表示目前封包沒有產生合格警告，不證明
整份任務契約已完成。
...
Provider 不會收到完整對話、模型未公開的內部思考或先前批次的工具結果。Masters'
Nudge 不會掃描工作區或讀取工作區外的檔案；只有任務明確寫出相對路徑的本機檔案
才會包含在封包內。
