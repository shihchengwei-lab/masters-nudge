...
其中 clap 題目的文字與驗收格式有歧義；兩次 Vue CSS 回饋的方向也有時序問題。這些結果顯示值得追查的結構改善，同時還不足以判定額外成本是否值得。詳見[正式報告](benchmark/formal-v8/ROUND-8-REPORT.zh-TW.md)。本輪測的是此分支尚未發布的提示詞，不代表已安裝的 `0.6.0` 版本。
