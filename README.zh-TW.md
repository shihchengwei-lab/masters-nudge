...
原始碼；不會尋找關聯檔案或保留原始碼歷史。Provider 自行形成因果判斷，再挑出最
可能改變下一個工程決策、且非顯而易見的觀察或精準問題。
...
一項事實；兩者都不成立時保持沉默。`anchor` 只保留一個最小定位；Host 顯示時才加上
固定的 `nudge`，例如 `causality nudge:`。`nudge` 不承擔分類或程度語意。
...
每則 Nudge 都依目前情況生成，不是隨機抽一句罐頭訊息。Nudge 是一則觀察或一個
能改變當前工程決策的精準問題，不是 review、評分、完整解法，也不是一律要求多跑
測試。
